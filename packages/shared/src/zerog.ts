...
// ─── Read-back: inheriting an Agentic ID's memory ─────────────────────────────
//
// The write side (mintAgenticId / updateAgenticIdMemory) is only half the loop.
// A new agent instance that owns a token should be able to *read* the memory
// pointer back off-chain and rehydrate — that is what makes the memory portable
// across a sale. `ZEROG_AGENT_TOKEN_ID` tells an instance "you own token N"; a
// boot-time read of intelligentData(N) yields the 0G Storage root to download.

/** The token this instance should inherit memory from, if any (env-configured). */
export function agentNftTokenId(): string | null {
  const t = process.env.ZEROG_AGENT_TOKEN_ID?.trim();
  return t && /^\d+$/.test(t) ? t : null;
}

/**
 * Explicit 0G Storage root override. A safety net for the read-back demo when the
 * on-chain dataHash isn't the recoverable root (mint keccak-folds any root that
 * isn't already 32-byte hex). Set it to the root logged at mint time.
 */
export function inheritedMemoryRoot(): string | null {
  const r = process.env.ZEROG_MEMORY_ROOT?.trim();
  return r ? r : null;
}

/**
 * Read an Agentic ID's on-chain intelligent-data. The stored `dataHash` is the
 * agent's encrypted-memory pointer in 0G Storage (equal to the 0G root when that
 * root is 32-byte hex). No-op (null) unless a deployed AgentNFT is configured;
 * never throws — a non-existent token or an offline node just returns null.
 */
export async function readAgenticIdMemory(
  tokenId: string,
): Promise<{ description: string; memoryRoot: string } | null> {
  const nft = agentNftAddress();
  if (!nft) return null;
  try {
    const pub = zeroGPublicClient();
    ...
    return { description: data.dataDescription, memoryRoot: data.dataHash };
  } catch (e) {
    log("0g-chain", `Agentic ID ${tokenId} read skipped (${(e as Error).message.slice(0, 120)})`);
    return null;
  }
}
