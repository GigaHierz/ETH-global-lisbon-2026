...
([src/index.ts](src/index.ts)) with four jobs:

1. **Discovery from the chain** ([src/discovery.ts](src/discovery.ts)): polls the HCS
...
3. **Payment** ([src/payer.ts](src/payer.ts)): the exchange holds its own wallet
...
- **Verifier is trusted** (it holds the escrow key). Production: verifier sets with their
  own stakes and dispute rounds; `contracts/Staking.sol` (written + tested, deliberately
  undeployed for the No-Solidity track) is the trustless upgrade path.
