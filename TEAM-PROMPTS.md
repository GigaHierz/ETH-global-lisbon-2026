...
**Install Superpowers (all three of you, once per laptop):** Superpowers is a Claude Code skills framework — TDD, systematic debugging, brainstorming, and subagent-driven development with built-in code review. See [claude.com/plugins/superpowers](https://claude.com/plugins/superpowers). Install it once with `/plugin install superpowers@claude-plugins-official` and restart Claude Code. After that it's active automatically from the first message of **every new session** — there's nothing to re-run. To start: just open a fresh session (`claude`) and describe your one feature in plain language; Superpowers activates the right skill for the task on its own.
