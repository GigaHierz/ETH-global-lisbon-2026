...
1. **x402 (open payment standard)** — HTTP-native agentic payments. Every inference request is individually paid in **USDC** (native HBAR behind `SETTLEMENT_ASSET=hbar`) on Hedera Testnet via the HTTP 402 status code. No accounts, no API keys, no subscriptions — an agent with a Hedera wallet can buy one single completion.
...
- **TypeScript pnpm monorepo**: `packages/` — `provider` `exchange` `verifier` `agent` `dashboard` (Next.js) `shared` `provider-mcp` (MCP server for agent-driven provider onboarding)
...
| Component | Status |
|---|---|
| x402 payment protocol | **Real** — official `@x402/*` v2.19 packages, real 402 challenges settled on `hedera:testnet` in **HTS USDC** (native HBAR behind `SETTLEMENT_ASSET=hbar`) via the hosted facilitator ladder (`api.testnet.blocky402.com` → `x402.org/facilitator`, both feePayer-sponsored). Verified settlements + balance deltas in PROOF.md |
...
| GPU / compute supply | **Partly real** — the demo providers proxy **Groq**; a fourth provider (**NimbusAI**) resells **0G Compute** (0G's decentralized GPU marketplace, `0gm-1.0-35b-a3b`), and its stake + HCS registration + a settled USDC trade are on-chain (PROOF.md). 0G completion *content* uses the canned fallback until `ZEROG_API_KEY` is funded. Compute backends are pluggable (`0g` default for bring-your-own, `groq`, `canned`) — we don't run our own GPUs; the marketplace + verification mechanics are the contribution |
| Provider onboarding (`provider-mcp`) | **Real** — an MCP server exposes account creation, staking, HCS-14 registration, and liveness checks as agent-callable tools; a developer (or agent) goes from fresh checkout to a live, registered, paid provider. Idempotent/resumable, ships as project-scoped `.mcp.json` |

## Questions she'll get, with answers

**"Is the money real?"**
Real USDC settling on Hedera Testnet via the hosted x402 facilitator (feePayer-sponsored, so payers need zero gas; native HBAR via `SETTLEMENT_ASSET=hbar`). Mainnet would be a config change (network string + facilitator), not an architecture change.
...
- **x402** — Coinbase's open protocol reviving HTTP status code 402 "Payment Required": server says what it costs, client pays (here in USDC, or native HBAR via `SETTLEMENT_ASSET=hbar`), request proceeds. Machine-to-machine payments over plain HTTP.
