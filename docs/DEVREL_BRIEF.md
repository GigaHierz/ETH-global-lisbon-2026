...
4. **Optimistic verification + staking + multi-sig scheduled enforcement** — providers stake HBAR collateral. A verifier randomly replays past prompts (temperature 0) against a second provider claiming the same model. If answers diverge, the cheater's staked HBAR is slashed, its bond is **frozen** and then **wiped via a 2-of-2 multi-sig scheduled transaction** (Hedera Schedule Service, no keeper), negative reputation is filed on-chain, and it's ejected from routing.
...
| **Multi-sig scheduled wipe** (Schedule Service) | **Real, SDK-native** — the bond wipe is a `ScheduleCreateTransaction` needing a 2-of-2 [verifier, auditor] signature; verifier signs on create, auditor `ScheduleSign`s → the wipe executes on-chain with no keeper (`packages/shared/src/hts.ts`). For the demo the verifier completes both signatures; production splits them |
...
**"Is the scheduled wipe really multi-sig?"**
The bond's wipe key is a 2-of-2 `KeyList` [verifier, auditor]. Destroying a bond therefore needs two signatures: the verifier creates a `ScheduleCreateTransaction` (signature 1), the auditor `ScheduleSign`s it (signature 2), and only then does the wipe execute — on-chain, no keeper or bot. For the demo the verifier completes both signatures so the trigger→execution loop runs end-to-end; in production the auditor is an independent second signer, which is also the honest answer to "who watches the verifier?".
