...
**What this is:** AgentRouter is an on-chain OpenRouter — an exchange where AI agents buy LLM inference per-request with USDC via x402, from providers whose identity, trades, and verification verdicts live on Hedera Consensus Service. Providers stake HBAR into escrow; a verifier replays sampled prompts against witnesses and slashes providers caught serving cheaper models than advertised. Everything runs on Hedera Testnet with mock mode as an offline-proof fallback.
...
flowchart LR
    ...
    E -->|"x402 USDC"| P1[Titan 70b · $0.10 :4021]
    E -->|"x402 USDC"| P2[Budget 8b · $0.04 :4022]
    E -->|"x402 USDC"| P3[Sketchy 😈 claims 70b · $0.08 :4023]
...
|---|---|---|
| Provider :402x | `GET /info` | name, model, price, wallet, agentId |
...
| | `GET /providers` | routing table (model, price, stakeHbar, reputation, live, slashed) |
...
| `PROVIDER_NAME` / `PROVIDER_MODEL` / `PROVIDER_PRICE` / `PROVIDER_PORT` | Custom Provider / 70b / `0.10` / `4025` | custom provider (`pnpm provider`) — list your own compute, no code edits |
...
> `provider4` (NimbusAI), is an *honest* 70b provider priced at $0.06 — cheaper than the
> cheater's $0.08. If you boot it, cheapest-first routing sends 70b traffic to NimbusAI, the
