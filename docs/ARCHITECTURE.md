...
agents buy inference per request, pay in **USDC over x402** (native HBAR behind
`SETTLEMENT_ASSET=hbar`; sub-second finality, low predictable fees), providers stake HBAR + hold
an **HTS ReputationBond**, and a verifier slashes fraud and **destroys the bond with a 2-of-2
multi-sig wipe** — all on **Hedera Testnet**, SDK-native across **HCS + HTS + Mirror Node**,
**no smart contract**.
...
agent goal ─▶ plan (Groq) ─▶ for each question:
    agent ──USDC price+fee (x402)──▶ exchange ──routes to cheapest──▶ provider ──▶ completion
...
- **Payments** are x402: an HTTP 402 challenge → the payer signs a USDC transfer (native HBAR
  behind `SETTLEMENT_ASSET=hbar`) → a facilitator settles it on Hedera (fee-sponsored) → the
  request proceeds. The 402 quote is dynamic (`price + fee`, pinned 60s), not a fixed amount.
...
|---|---|
| **Hedera SDK** (`@hiero-ledger/sdk`) | `TransferTransaction` (x402 payments — USDC by default, native HBAR via `SETTLEMENT_ASSET=hbar` — plus 50 ℏ stakes, 25 ℏ slash), `TopicMessageSubmitTransaction` / `TopicCreateTransaction` (HCS), `TokenCreateTransaction` / `TokenAssociateTransaction` / `TokenFreezeTransaction` / `TokenWipeTransaction` (HTS bond), `KeyList` (2-of-2 wipe key) / `CustomFractionalFee`, `AccountCreateTransaction` + `setECDSAKeyWithAlias`, `AccountBalanceQuery`, `PrivateKey.fromStringECDSA` |
...
| Monorepo | **pnpm workspaces** (`agent`, `provider`, `exchange`, `verifier`, `shared`, `dashboard`, `provider-mcp`) |
| LLM | **Groq** (`llama-3.3-70b-versatile` / `llama-3.1-8b-instant`) — the demo providers' inference + the agent's reasoning brain |
| Compute backends | Pluggable per provider: **0G Compute** (default for bring-your-own supply, `0gm-1.0-35b-a3b`), **Groq**, or a **canned** offline fallback |
| Servers | **Express** + **Server-Sent Events** (agent-server, exchange) |
