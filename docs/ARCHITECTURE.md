...
| **Hedera Token Service (HTS)** | The **ReputationBond** (`ARBOND`) — fungible token created via SDK with a **custom fractional fee** + **freeze/pause/wipe compliance controls**; balance = on-chain reputation (`scripts/setup-hts-token.ts`, `packages/shared/src/hts.ts`) |
